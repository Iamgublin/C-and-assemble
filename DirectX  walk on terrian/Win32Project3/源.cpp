...
float getverheight(int x, int z);
...
void keycon()
{
	...
	_pos.y = getheight(_pos.x, _pos.z) + 5;
}
float getheight(float x, float z)
{
	x = (te.getwidth() / 2) + x;
	z = (te.getdepth() / 2) - z;
	x = x / te.distanceperpointx;
	z = z / te.distanceperpointz;
	float col = floorf(x);
	float row = floorf(z);
	float dx = x - col;
	float dz = z - row;
	float A = getverheight((int)col, (int)row);
	float B = getverheight((int)col+1, (int)row);
	float C = getverheight((int)col, (int)row+1);
	float D = getverheight((int)col+1, (int)row+1);
	float height = 0.0f;
