...
#define TM012 300
#define DIST 60
    int i,j,k,s,t=boss_shot.cnt%TM012,t2=boss_shot.cnt;
    double angle,x,y;
    static int num;
    if(t2==0)num=0;
    //60カウント以下で10カウントに1回
    ...
    if(t2%2==0){
		int set_x[4]={-90,-40,40,90},set_y[4]={-30,30,30,-30};
		double tmangle=PI/6;
		for(j=0;j<4;j++){
			x=boss.x+set_x[j];
			y=boss.y+set_y[j];
			...
			for(i=0;i<8;i++){
				if((k=search_boss_shot())!=-1){
					boss_shot.bullet[k].col   = 0;//弾の色
					boss_shot.bullet[k].x     = boss.x+set_x[j];//座標
					boss_shot.bullet[k].y     = boss.y+set_y[j];
					boss_shot.bullet[k].knd   = 8;//弾の種類
					boss_shot.bullet[k].angle = angle+tmangle/2-tmangle*4+tmangle*i+sin(PI2/25*t)*PI/70;//角度
					boss_shot.bullet[k].flag  = 1;
					boss_shot.bullet[k].cnt   = 0;
					boss_shot.bullet[k].spd   = 6;//スピード
					boss_shot.bullet[k].eff   = 0;
					boss_shot.bullet[k].state = 0;
				}
			}
	...
	if(t2%9==0 && t2>60){
		...
		for(i=0;i<15;i++){
			if((k=search_boss_shot())!=-1){
				...
				boss_shot.bullet[k].x     = boss.x;//座標
				boss_shot.bullet[k].y     = boss.y;
				boss_shot.bullet[k].knd   = 13;//弾の種類
				boss_shot.bullet[k].angle = angle-tmangle*7+tmangle*i;//角度
				boss_shot.bullet[k].flag  = 1;
				boss_shot.bullet[k].cnt   = 0;
				boss_shot.bullet[k].spd   = 3;//スピード
				boss_shot.bullet[k].eff   = 0;
    ...
    if(t==120){
        ...
        if((k=search_lazer())!=-1){
            lazer[k].col      = 0;//弾の色
            lazer[k].knd      = 0;//弾の種類
            lazer[k].angle    = bossatan2()-PI/7*plmn;//角度
